...
export const storeLogo = async (req: Request, res: Response): Promise<Response> => {
  ...
  if (validModes.indexOf(mode) === -1) {
    return res.status(406).json({ error: "Modo inválido" });
  }

  if (file && file.mimetype.startsWith("image/")) {
    // Garante que só o nome do arquivo será salvo (sem path ou URL)
    let filename = file.filename;
    // Remove qualquer path ou URL acidentalmente presente
    if (filename.includes("/")) filename = filename.split("/").pop();
    if (filename.includes("\\")) filename = filename.split("\\").pop();
    if (filename.startsWith("http")) {
      // Se vier uma URL completa, pega só o nome do arquivo
      try {
  ...
  return res.status(406).json({ error: "Arquivo inválido" });
}
