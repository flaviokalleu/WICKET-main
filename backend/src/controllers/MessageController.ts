// Recebe chunks de áudio e monta arquivo final
...
export const storeAudioChunk = async (req: Request, res: Response) => {
  try {
    const { ticketId } = req.params;
    const chunk = req.file;
    if (!chunk) return res.status(400).json({ error: "Nenhum chunk recebido" });
    const chunkFile = path.join(chunkDir, `${ticketId}_${Date.now()}_${Math.random().toString(36).slice(2)}.webm`);
    fs.writeFileSync(chunkFile, chunk.buffer);
    // Aqui você pode implementar lógica para juntar os chunks depois
