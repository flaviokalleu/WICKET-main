...
// Extend IContextInfo to include ticketId
  ...
  try {
    ...
    const types = {
      ...
      eventMessage: msg?.message?.eventMessage?.name
        ...
        : "sem nome do evento\n",
      orderMessage: msg.message?.orderMessage?.orderTitle || "[Pedido]"
    };
...
export const verifyMediaMessage = async (
  ...
  if (!ticket) {
    logger.warn("[verifyMediaMessage] Ticket is null or undefined. Skipping media message processing.", { msgId: msg?.key?.id });
    return;
  }
  const io = getIO();
...
export const verifyMessage = async (
  ...
  if (!ticket) {
    logger.warn("[verifyMessage] Ticket is null or undefined. Skipping message processing.", { msgId: msg?.key?.id });
    return;
  }
  const io = getIO();
  const quotedMsg = await verifyQuotedMessage(msg);
...
const isValidMsg = (msg: proto.IWebMessageInfo): boolean => {
  ...
  try {
    ...
    const ifType =
      ...
      msgType === "orderMessage"; // Adicionado para tratar mensagens de pedidos
...
const wbotMessageListener = (wbot: Session, companyId: number): void => {
  wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
    ...
    messages.forEach(async (message: proto.IWebMessageInfo) => {
      ...
      // Busca mensagem considerando ticketId para garantir unicidade correta
      let ticketIdForCount = undefined;
      // Tenta obter ticketId do contexto da mensagem, se disponível
      if (message?.message?.extendedTextMessage?.contextInfo?.ticketId) {
        ticketIdForCount = message.message.extendedTextMessage.contextInfo.ticketId;
      }
      // Se não conseguir ticketId, mantém filtro antigo (fallback)
      const countWhere = ticketIdForCount
        ? { wid: message.key.id!, ticketId: ticketIdForCount, companyId }
        : { wid: message.key.id!, companyId };
