...
// Funções auxiliares para construir caminhos de mídia de forma consistente
const buildMediaPath = (fileName: string): string => {
  // Remove "uploads/" do início do fileName se existir para evitar duplicação
...
// Função alternativa mais robusta para construir caminhos
const buildMediaPathRobust = (fileName: string): string => {
  // Remove qualquer prefixo de diretório que já contenha uploads
  ...
  // Remove "public/uploads/" do início se existir
  if (cleanFileName.startsWith("public/uploads/")) {
    cleanFileName = cleanFileName.substring(15);
  }
  ...
  const fullPath = path.join(basePath, "public", "uploads", cleanFileName);
  
  console.log(`[buildMediaPath] fileName original: ${fileName}`);
  console.log(`[buildMediaPath] fileName limpo: ${cleanFileName}`);
  console.log(`[buildMediaPath] caminho final: ${fullPath}`);
...
export const ActionsWebhookService = async (
  ...
  try {
    ...
    for (var i = 0; i < lengthLoop; i++) {
      ...
      // Adiciona/remover tag do ticket e do contato se o nó for do tipo tag
      if (nodeSelected.type === "tag" && ticket) {
        try {
          if (!nodeSelected.data.tag) {
            console.error("Tag não definida no nó de tag");
          } else {
            // Busca a tag pelo nome e companyId
            let tag = await Tag.findOne({ where: { name: nodeSelected.data.tag, companyId } });
            if (!tag && nodeSelected.data.action === "add") {
              // Cria a tag se não existir e for para adicionar
              tag = await Tag.create({ name: nodeSelected.data.tag, color: "#3ABA38", companyId });
            }
            if (tag) {
              // Busca o contato relacionado ao ticket
              const contact = await Contact.findOne({ where: { id: ticket.contactId } });
              if (nodeSelected.data.action === "add") {
                // Adiciona relação ticket-tag se não existir
                const exists = await TicketTag.findOne({ where: { ticketId: ticket.id, tagId: tag.id } });
                if (!exists) {
                  await TicketTag.create({ ticketId: ticket.id, tagId: tag.id });
                }
                // Adiciona relação contact-tag se não existir
                if (contact) {
                  const existsContactTag = await ContactTag.findOne({ where: { contactId: contact.id, tagId: tag.id } });
                  if (!existsContactTag) {
                    await ContactTag.create({ contactId: contact.id, tagId: tag.id });
                  }
                }
              } else if (nodeSelected.data.action === "remove") {
                // Remove relação ticket-tag se existir
                await TicketTag.destroy({ where: { ticketId: ticket.id, tagId: tag.id } });
                // Remove relação contact-tag se existir
                if (contact) {
                  await ContactTag.destroy({ where: { contactId: contact.id, tagId: tag.id } });
                }
              }
            }
          }
        } catch (err) {
          console.error("Erro ao processar tag no fluxo:", err);
        }
      }
      ...
      if (nodeSelected.type === "singleBlock") {
        for (var iLoc = 0; iLoc < nodeSelected.data.seq.length; iLoc++) {
          ...
          if (elementNowSelected.includes("img")) {
            ...
            const mediaDirectory = buildMediaPathRobust(fileName);

            if (!checkFileExists(mediaDirectory)) {
              console.error("Arquivo de imagem não encontrado:", mediaDirectory);
              continue;
            }
          ...
          if (elementNowSelected.includes("audio")) {
            ...
            if (!checkFileExists(mediaDirectory)) {
              console.error("Arquivo de áudio não encontrado:", mediaDirectory);
              continue;
            }
          ...
          if (elementNowSelected.includes("video")) {
            ...
            if (!checkFileExists(mediaDirectory)) {
              console.error("Arquivo de vídeo não encontrado:", mediaDirectory);
              continue;
            }
      ...
      if (nodeSelected.type === "file") {
        ...
        const mediaDirectory = buildMediaPathRobust(nodeSelected.data.url);
