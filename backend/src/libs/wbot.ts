...
export const initWASocket = async (whatsapp: Whatsapp): Promise<Session> => {
  ...
  `})
            ...
            const updatedWpp = await Whatsapp.findByPk(whatsapp.id);
            logger.info(`[IMPORT] statusImportMessages atualizado: ${updatedWpp?.statusImportMessages}`);
            // LOGS DE DEPURAÇÃO DO FLUXO DE IMPORTAÇÃO
            console.log('[DEBUG] statusImportMessages setado:', statusImportMessages);
            console.log('[DEBUG] wpp.status:', wpp.status);
            console.log('[DEBUG] wpp.importOldMessages:', wpp.importOldMessages);
            console.log('[DEBUG] wpp.importRecentMessages:', wpp.importRecentMessages);
            console.log('[DEBUG] Aguardando evento messaging-history.set para continuar importação...');

            // FORÇAR IMPORTAÇÃO CASO O EVENTO NÃO OCORRA EM 60 SEGUNDOS
            setTimeout(async () => {
              const wppCheck = await Whatsapp.findByPk(whatsapp.id);
              if (wppCheck?.importOldMessages && wppCheck.status === "CONNECTED") {
                // Verifica se statusImportMessages ainda está como timestamp (não mudou para 'Running')
                if (typeof wppCheck.statusImportMessages === 'number' || !isNaN(Number(wppCheck.statusImportMessages))) {
                  addLogs({
                    fileName: `preparingImportMessagesWppId${whatsapp.id}.txt`,
                    text: `[FORCE] Forçando importação por timeout: evento messaging-history.set não disparou em 60s.`
                  });
                  console.log('[FORCE] Forçando importação: evento messaging-history.set não disparou em 60s.');
                  ImportWhatsAppMessageService(wppCheck.id);
                  await wppCheck.update({ statusImportMessages: "Running" });
                }
              }
            }, 60000);
            wsocket.ev.on("messaging-history.set", async (messageSet: any) => {
              ...
              console.log(`[IMPORT] Total de mensagens recebidas do WhatsApp:`, filteredMessages.length);
              addLogs({
                fileName: `preparingImportMessagesWppId${whatsapp.id}.txt`,
                text: `[IMPORT] Total de mensagens recebidas do WhatsApp: ${filteredMessages.length}`
              });
              ...
              console.log(`[IMPORT] Total de mensagens dentro do filtro de datas:`, filteredDateMessages.length);
              addLogs({
                fileName: `preparingImportMessagesWppId${whatsapp.id}.txt`,
                text: `[IMPORT] Total de mensagens dentro do filtro de datas: ${filteredDateMessages.length}`
              });
