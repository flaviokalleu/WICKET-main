...
const DonutChart = (props) => {
  ...
  const renderCustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index }) => {
    ...
    return (
      <text x={x} y={y} fill="#ffffff" textAnchor='middle' dominantBaseline="middle" fontWeight="bold">
        <tspan fontWeight="bold" fontSize="14">{`${title}`}</tspan> 
        <tspan x={x} y={y} dy="1.2em" fontWeight="bold" fontSize="16" fill="#00d4aa">{`${value}%`}</tspan>
      </text>
    );
  };
